...
test("buildAgentCommand can append a thread-specific system prompt for Claude", () => {
  const command = buildAgentCommand("claude", makeStream(), "working", {
    appendSystemPrompt: "You are working in thread b-123. Always pass threadId: b-123.",
  });
  expect(command).toContain("--append-system-prompt");
  expect(command).toContain("thread b-123");
});
...
test("buildAgentCommandForSession with empty resumeSessionId is identical regardless of which session was previously saved", () => {
  ...
  // The stripped form is a stable "launcher identity" for the thread: same
