...
function registerIpc(currentRuntime: ElectronRuntime) {
  ...
  handle("oxplow:recordPageVisit", (_event, input) => currentRuntime.recordPageVisit(input));
  ...
  handle("oxplow:getRepoConflictState", (_event, streamId: string) => currentRuntime.getRepoConflictState(streamId));
