...
function registerIpc(currentRuntime: ElectronRuntime) {
  ...
  ipcMain.handle("newde:openTerminalSession", (_event, paneTarget: string, cols: number, rows: number) =>
    currentRuntime.openTerminalSession(paneTarget, cols, rows, (sessionId, message) => {
