---
description: Review this session's transcript and propose concrete oxplow improvements
---

# /self-reflect — reflect on the session, propose oxplow upgrades

You are going to review **your own transcript** for the current thread and produce a focused critique of oxplow itself. The goal is to find changes to oxplow's code, prompts, tooling, or UX that would make future sessions faster, cheaper, more correct, and more collaborative.

Remember: oxplow is a shipped product that other users run too. Fixes belong in oxplow's code, prompts, hooks, or docs — not in your auto-memory.

## Step 1 — Read the transcript

- Use `ls ~/.claude/projects/<YOUR DIR>/*.jsonl -t | head -1` (adjust the project slug if the cwd differs) to find the most recent transcript file, or pull `sessionId` from `oxplow__get_batch_context` and read `~/.claude/projects/<project-slug>/<sessionId>.jsonl`.
...
- Also call `oxplow__list_agent_turn` and `oxplow__list_batch_work` for this thread so your analysis is grounded in what actually shipped vs what was asked.
