...
| `apps/desktop/src/pages/DiffViewPage.tsx` | The **explicit start→end diff view** (`diff-view` kind). Three modes via `DiffViewSpec`: `snapshot` (legacy `snapshotRef(N)` prev→N drill-in — full function/duplication analysis, behavior unchanged from the former SnapshotDetailPage); `effort` (`effortDiffRef(effortId)` — resolves the effort's own start/end snapshot bracket via the `get_effort` IPC on load, with the task-title link + an "Effort is in progress" notice when the end is null); `endpoints` (`endpointDiffRef(start, end)` — an explicit snapshot/commit/working pair). The effort/endpoint modes route the file list through `diff_endpoints` (unified substrate) via `useChangeAnalysis({ endpoints })`, render the diff-oriented header + `SummaryCard`, group changed files by the efforts overlapping the range (`listEffortsOverlappingRange` → `groupChangesByEffort`), and hide the effort roster for pure commit↔commit ranges. Pure model logic (`resolveEffortEndpoints`, `endpointLabel`, `snapshotRange`, `isPureCommitRange`) lives React-free in `apps/desktop/src/diffViewModel.ts`. The `diff_endpoints` substrate (tsk339) handles mixed snapshot↔commit (normalizing the snapshot side into git-oid space), the working-tree endpoint, and per-file line counts (via `similar`); deep per-function analysis for endpoint mode is still snapshot-mode-only. |
