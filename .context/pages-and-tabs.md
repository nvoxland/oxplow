...
| `apps/desktop/src/tabs/PageNavBar.tsx` | Dumb nav-bar component: back/forward buttons, optional bookmark toggle, optional backlinks/outbound/snapshots dropdowns (popovers), an optional **prose-audience selector** slot (`audience`, a `ProseAudienceSelector` segmented control rendered before the comment navigator on variant-bearing pages), and an optional **comment navigator** slot (`comments` ReactNode, rendered before Backlinks). Mounted by `Page` when context or explicit `navBar` prop is present. |
...
`crates/oxplow-domain/src/prose.rs`) render a segmented
**audience selector** in the nav bar. The selection is **per-page**,
keyed by the same `pageKey` (`${threadId}::${tabId}`), and lives in a
dedicated external store (`apps/desktop/src/tabs/proseAudience.ts` +
`useProseAudience.ts`) modeled on `bookmarks.ts` — a single
localStorage blob `oxplow.prose-audience.v1`, default `developer`,
`set("developer")` removes the row. `closePageTab` clears the row
alongside the snapshot. The nav-bar selector
(`ProseAudienceSelector`) is wired through an optional `audience` prop
