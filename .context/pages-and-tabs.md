...
| `apps/desktop/src/components/RailHud/RailHud.tsx` | Persistent left rail HUD: a pinned search trigger at the top, then a set of **uniform collapsible sections** (Uncommitted, Comments, Work, Errors, Bookmarks, History). Each renders through the shared `RailSection` wrapper — drag handle (⠿) + expand/collapse chevron + title + optional count badge + optional header action (History's top/recent, Errors' clear). Sections **drag-to-reorder** (MIME `application/x-oxplow-rail-section`, order persisted in `localStorage` `oxplow.rail.sectionOrder`) and each section's **expanded/collapsed state persists per thread** (`oxplow.rail.sectionExpanded.v1` keyed by threadId). Work defaults collapsed and keeps a one-line summary (active item / last done) when collapsed; the rest default expanded and hide their body when collapsed. A section that has no content returns null and simply doesn't render. Passive — never auto-opens tabs. Bookmark rows show a single-letter scope badge (T/S/G) and no remove button (unstar from the page). |
...
6. **Bookmarks** — the user-curated pinned set. There is **no "Pages"
...
7. **History** — recent / most-visited pages.
