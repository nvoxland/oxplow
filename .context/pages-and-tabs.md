...
| `apps/desktop/src/pages/DashboardPage.tsx` | Composite Planning / Review / Quality dashboards plus the **Go To** page. Variant chosen via `dashboardRef("planning"\|"review"\|"quality"\|"visits")`. The `visits` variant is the **Go To** page (titled "Go To"): a navigation hub combining the user's **managed bookmarks** (open + inline scope segmented control via `bookmarks.setScope` + remove with Undo toast), a **toggle-able Recently Visited / Most Visited list** (`VisitsBrowser`, segmented control, default Recently Visited), and a visits-per-day chart. It's the page the rail's combined Bookmarks pane links to; needs `threadId` (passed from `App.tsx`) to scope bookmark reads/writes. |
