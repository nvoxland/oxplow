...
effort but weren't claimed. Some are genuinely yours (a file you edited
through Bash, which is never auto-claimed). Many are **build output**,
and those keep reappearing.

Tells that a flagged path is generated rather than authored:
...
Don't just disclaim it each time — that hides a fixable problem and you
will pay it again next effort. **Tell the user and offer to add it to
`generated.exclude` in `.oxplow/project.yaml`.** Once declared, the path
stops being captured, snapshotted, and flagged at all.

Say what you observed and let them decide, e.g.: *"`…/generated/bindings.ts`
has been flagged in most of the last several efforts. It's Tauri Specta
